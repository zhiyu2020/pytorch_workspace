...
def compute_integral_part(input, overflow_rate):
    abs_value = input.abs().view(-1) # [-1] show 1 x [-1]
    sorted_value = abs_value.sort(dim=0, descending=True)[0] # show values descending
...
def linear_quantize(input, sf, bits):
    ...
    clipped_value = torch.clamp(rounded, min_val, max_val) * delta # take tensor into new range and get int value
